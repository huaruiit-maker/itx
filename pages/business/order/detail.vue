<template>
  <view class="business-order-detail">
    <!-- 订单状态 -->
    ...
    <view class="info-card">
      ...
      <view class="info-item" v-if="orderInfo.assignTime">
        <text class="item-label">分配时间</text>
        <text class="item-value">{{ orderInfo.assignTime }}</text>
      </view>
    ...
    <!-- 师傅信息 -->
    <view class="info-card" v-if="orderInfo.masterInfo">
      <view class="card-title">师傅信息</view>
      <view class="master-info">
        <image :src="orderInfo.masterInfo.avatar" mode="aspectFill"></image>
        <view class="master-details">
          <text class="master-name">{{ orderInfo.masterInfo.name }}</text>
          <view class="master-rating">
            <u-rate v-model="orderInfo.masterInfo.rating" size="20" readonly></u-rate>
            <text class="rating-text">{{ orderInfo.masterInfo.rating }}分</text>
          </view>
          <text class="master-desc">服务次数：{{ orderInfo.masterInfo.serviceCount }}次</text>
        </view>
        <view class="master-actions">
          <u-button size="mini" plain @tap="callMaster">联系师傅</u-button>
        </view>
      </view>
    </view>
    ...
    <view class="footer-actions">
      <view class="action-btn" @tap="contactCustomer">
      ...
      <template v-else-if="orderInfo.status === 2">
        <u-button type="primary" @tap="assignMaster">分配师傅</u-button>
      </template>
      
      <template v-else-if="orderInfo.status === 3">
        <u-button type="default" plain @tap="contactMaster">联系师傅</u-button>
        <u-button type="primary" @tap="viewSchedule">查看日程</u-button>
      </template>
      
      <template v-else-if="orderInfo.status === 4">
        <u-button type="primary" @tap="confirmComplete">确认完成</u-button>
      </template>
      
      <template v-else-if="orderInfo.status === 5">
        <u-button type="default" plain @tap="viewEvaluation">查看评价</u-button>
        <u-button type="primary" @tap="createOrder">再次下单</u-button>
      </template>
    </view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    assignMaster() {
      uni.navigateTo({
        url: `/pages/business/master/assign?orderId=${this.orderId}`
      })
    },
    ...
    viewSchedule() {
      if (this.orderInfo.masterInfo) {
        uni.navigateTo({
          url: `/pages/master/schedule/manage?masterId=${this.orderInfo.masterInfo.id}`
        })
      }
    },
    ...
    viewEvaluation() {
      uni.navigateTo({
        url: `/pages/business/evaluation/detail?orderId=${this.orderId}`
      })
    },
    ...
    createOrder() {
      uni.navigateTo({
        url: `/pages/customer/service/detail?id=${this.orderInfo.serviceId}`
      })
    },
